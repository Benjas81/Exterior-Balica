...
// Pano2VR 6.1.2/17873
// Filename: benjas 02.ggsk
// Generated 2024-01-15T13:46:10
...
function pano2vrSkin(player,base) {
	...
	this.addSkin=function() {
		...
		me._plantas.logicBlock_visible = function() {
			...
			if (me._plantas.ggCurrentLogicStateVisible != newLogicStateVisible) {
				me._plantas.ggCurrentLogicStateVisible = newLogicStateVisible;
				me._plantas.style[domTransition]='opacity 0s';
				if (me._plantas.ggCurrentLogicStateVisible == 0) {
					me._plantas.style.visibility="hidden";
					me._plantas.ggVisible=false;
				}
				else {
					me._plantas.style.visibility=(Number(me._plantas.style.opacity)>0||!me._plantas.style.opacity)?'inherit':'hidden';
					me._plantas.ggVisible=true;
				}
			}
		}
		me._plantas.logicBlock_alpha = function() {
			...
			if (
				...
				((me.ggUserdata.tags.indexOf("iNTERIOR") == -1))
			)
			{
		...
		me._planta.ggUpdateConditionTimer=function () {
			me._planta.ggRadar.update();
		}
		me._planta.ggUpdatePosition=function (useTransition) {
		}
		me._planta.ggNodeChange=function () {
			if (me._planta.ggLastActivMarker) {
				if (me._planta.ggLastActivMarker._div.ggDeactivate) me._planta.ggLastActivMarker._div.ggDeactivate();
			}
			var id=player.getCurrentNode();
			if (me.ggMarkerArray) {
			var marker=me._planta.ggMarkerArray[id];
			if (marker) {
				if (marker._div.ggActivate) marker._div.ggActivate();
			}
			me._planta.ggLastActivMarker=marker;
			}
			if (!me._planta.ggMapNotLoaded) {
				me._planta.ggCenterNode();
			}
			if (player.getMapType(me._planta.ggMapId) == 'file') {
				var coords = player.getNodeMapCoords(id, me._planta.ggMapId);
				if (coords.length < 2) {
					var mapId = player.getMapContainingNode(id);
					if (mapId != '') {
							me._planta.ggChangeMap(mapId);
					}
				}
			}
			me._planta.ggLastNodeId = id;
		}
		me._plantas.appendChild(me._planta);
		me.divSkin.appendChild(me._plantas);
		me._planta.ggMarkerInstances=[];
		me._planta.ggMapId = 'FloorPlan01';
		me._planta.ggLastNodeId=null;
		me._planta.ggMarkerArray=[];
		me._planta.ggGoogleMarkerArray=[];
		me._planta.ggLastZoom = -1;
		me._planta.ggRadar={ lastFov : -1, lastPan : -1, lastZoom : -1,activeNodeLatLng : null, poly : null }
		me._planta.ggRadar.update=function() {
			var radar=me._planta.ggRadar;
			var map=me._planta.ggMap;
			if (!map) return;
			var d2r = Math.PI/180 ;
			var r2d = 180/Math.PI ;
			...
			var gps;
			if (player.getMapType(me._planta.ggMapId) == 'web') {
				gps=player.getNodeLatLng();
			} else {
				gps=player.getNodeMapCoords(null, me._planta.ggMapId);
				pan -= me._planta.ggFloorplanNorth;
			}
			var filterpassed = true;
			var currentId = player.getCurrentNode();
			if (me._planta.ggFilteredIds.length > 0 && me._planta.ggFilteredIds.indexOf(currentId) == -1) filterpassed = false;
		...
		me._planta.ggTileAvailable=function(x, y, z) {
			var mapDetails = player.getMapDetails(me._planta.ggMapId);
		...
		me._planta.ggInitMap=function(keepZoom) {
			me._planta.ggMapNotLoaded = false;
			var mapType = player.getMapType(me._planta.ggMapId);
			var mapDetails = player.getMapDetails(me._planta.ggMapId);
			if (mapType == 'file') {
				me._planta.style.backgroundColor = mapDetails['bgcolor'];
				me._planta.ggFloorplanNorth = mapDetails['floorplannorth'];
			} else {
				me._planta.style.backgroundColor = '#fff';
			}
			var gps;
			if (player.getMapType(me._planta.ggMapId) == 'web') {
				gps=player.getNodeLatLng();
			...
			if (mapType == 'web') {
				if (me._planta.ggLastZoom == -1) me._planta.ggLastZoom = 14;
				var initZoom = keepZoom ? me._planta.ggLastZoom : 14;
				...
				me._planta.ggMap = L.map(me._planta, mapOptions).setView(activeNodeLatLng, initZoom);
				if (mapDetails['mapprovider'] == 'openstreetmap') {
					if (mapDetails['mapstyle'] == 'streets') {
						L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{}).addTo(me._planta.ggMap);
					} else if (mapDetails['mapstyle'] == 'outdoors') {
						L.tileLayer('https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png',{}).addTo(me._planta.ggMap);
					}
				} else if (mapDetails['mapprovider'] == 'mapbox') {
					if (mapDetails['styleurl'] == '') {
						L.tileLayer('https://api.mapbox.com/v4/mapbox.' + mapDetails['mapstyle'] +  '/{z}/{x}/{y}@2x.png?access_token=' + mapDetails['mapkey'],{}).addTo(me._planta.ggMap);
					} else {
						var styleurlstring = mapDetails['styleurl'];
						styleurlstring = styleurlstring.slice(styleurlstring.indexOf('styles/') + 7);
						L.tileLayer('https://api.mapbox.com/styles/v1/' + styleurlstring + '/tiles/256/{z}/{x}/{y}@2x?access_token=' + mapDetails['mapkey'],{}).addTo(me._planta.ggMap);
					}
				} else if (mapDetails['mapprovider'] == 'custom') {
					L.tileLayer(mapDetails['mapurltemplate'],{ maxZoom: mapDetails['mapmaxzoom']}).addTo(me._planta.ggMap);
				}
			} else if (mapType == 'file') {
				if (me._planta.ggLastZoom == -1) me._planta.ggLastZoom = 7;
				var initZoom = keepZoom ? me._planta.ggLastZoom : 7;
				var mapOptions = {
					zoom: initZoom,
					minZoom: 7,
				...
				var MapLayer = L.TileLayer.extend({
					getTileUrl: function(coords){
						if (me._planta.ggTileAvailable(coords.x, coords.y, coords.z)) {
							return basePath + 'images/maptiles/' + me._planta.ggMapId + '/' + coords.z + '/' + coords.x + '_' + coords.y + '.' + mapDetails['tileformat'];
						} else {
							return '';
						}
					}
				...
				mapLayer.addTo(me._planta.ggMap);
				me._planta.ggMap.on('move zoom', function() {
					me._planta.ggCheckBounds(mapDetails);
				});
				me._planta.ggCheckBounds(mapDetails);
			}
		}
		me._planta.ggClearMap=function() {
		if (me._planta.ggMap) me._planta.ggMap.remove();
		me._planta.ggMap = null;
		me._planta.ggClearMapMarkers();
		me._planta.ggMapNotLoaded = true;
		}
		me._planta.ggClearMapMarkers=function() {
			me._planta.ggLastActivMarker = null;
			var id,marker;
			var markers=me._planta.ggMarkerArray;
			for (id in markers) {
				if (markers.hasOwnProperty(id)) {
					marker=markers[id];
					marker.removeFrom(me._planta.ggMap);
				}
			}
			me._planta.ggMarkerArray=[];
			me._planta.ggMarkerInstances=[];
		...
		me._planta.ggCenterNode=function() {
			if (!me._planta.ggMap) return;
			var gps;
			if (player.getMapType(me._planta.ggMapId) == 'web') {
				gps=player.getNodeLatLng();
			} else {
			...
			if ((gps.length>=2) && ((gps[0]!=0) || (gps[1]!=0))) {
				...
				me._planta.ggMap.panTo(markerLocation, {animate: false});
			}
		}
		me._planta.ggFitBounds=function(force) {
			if (me._planta.ggMarkerBounds.isValid()) {
				if (me._planta.ggMarkerInstances.length > 1 || Object.getOwnPropertyNames(me._planta.ggGoogleMarkerArray).length > 1) {
					me._planta.ggMap.zoomOut(1, {animate: false});
					me._planta.ggMap.fitBounds(me._planta.ggMarkerBounds, {padding: [30, 30], animate: false});
				} else {
					me._planta.ggMap.setView(me._planta.ggMarkerBounds.getCenter(), me._planta.ggMap.getZoom());
					if (player.getMapType(me._planta.ggMapId) == 'web') {
						me._planta.ggMap.setZoom(18);
					} else {
						me._planta.ggMap.setZoom(7);
					}
				}
			}
		}
		me._planta.ggInitMapMarkers=function(updateMapBounds) {
			...
			me._planta.ggFilteredIds = [];
			if (me._planta.ggFilter != '') {
				var filter = me._planta.ggFilter.split(',');
				for (i=0; i < ids.length; i++) {
					var nodeId = ids[i];
					var nodeData = player.getNodeUserdata(nodeId);
					for (var j=0; j < filter.length; j++) {
						if (nodeData['tags'].indexOf(filter[j].trim()) != -1) me._planta.ggFilteredIds.push(nodeId);
					}
				}
				if (me._planta.ggFilteredIds.length > 0) ids = me._planta.ggFilteredIds;
			}
			var marker;
			var markerLocation;
			me._planta.ggMarkerBounds = L.latLngBounds();
			var currentId = player.getCurrentNode();
			for(var i=0;i<ids.length;i++) {
				...
				if (player.getMapType(me._planta.ggMapId) == 'web') {
					...
					gps=player.getNodeMapCoords(id, me._planta.ggMapId);
				}
				if ((gps.length>=2) && ((gps[0]!=0) || (gps[1]!=0))) {
					markerLocation = L.latLng(gps[0], gps[1]);
					var markerParent = new Object();
					markerParent.ggElementNodeId=function() { return id };
					var div=new SkinElement_marcador_negro_Class(me, markerParent);
					marker=new L.SkinMarkerLayer(div._marcador_negro,markerLocation);
					marker.addTo(me._planta.ggMap);
					me._planta.ggMarkerArray[id]=marker;
					me._planta.ggMarkerInstances.push(div);
					me._planta.ggMarkerBounds.extend(markerLocation);
				}
			}
			if (ids.length > 1 && me._planta.ggMarkerBounds.isValid() && updateMapBounds) {
				me._planta.ggFitBounds(false);
			}
			skin.updateSize(me._planta);
			this.ggLastActivMarker = null;
			if (this.ggUpdateConditionNodeChange) this.ggUpdateConditionNodeChange();
			this.ggRadar.lastFov = -1;
		}
		me._planta.ggChangeMap=function(mapId) {
			var newMapType = player.getMapType(mapId)
			if (newMapType == 'web') {
				return;
			}
			if (me._planta.ggMap) {
				me._planta.ggLastZoom = me._planta.ggMap.getZoom();
			}
			me._planta.ggMapId = mapId;
			me._planta.ggClearMap();
			me._planta.ggInitMap(true);
			me._planta.ggInitMapMarkers(false);
		var mapDetails = player.getMapDetails(me._planta.ggMapId);
		me._planta.ggCheckBounds(mapDetails);
		}
		me._planta.ggInCheckBounds=false;
		me._planta.ggCheckBounds=function(mapDetails) {
			var mapAR = mapDetails['width'] / mapDetails['height'];
			...
			if (me._planta.ggInCheckBounds) return;
			me._planta.ggInCheckBounds = true;
			var mapCenter = me._planta.ggMap.getCenter();
			var currentZoom = me._planta.ggMap.getZoom();
			var pixelInDeg = 360.0 / (Math.pow(2, currentZoom) * 256)
			var xOffset = (me._planta.clientWidth / 2.0) * pixelInDeg;
			var yOffset = (me._planta.clientHeight / 2.0) * pixelInDeg;
			var x = mapCenter.lng;
			var y = mapCenter.lat;
			if (mapWidthInDeg < me._planta.clientWidth * pixelInDeg) {
				x = mapWidthInDeg / 2;
			} else {
			if (x > mapWidthInDeg - xOffset) x = mapWidthInDeg - xOffset;
			if (x < xOffset) x = xOffset;
			}
			if (mapHeightInDeg < me._planta.clientHeight * pixelInDeg) {
				y = -mapHeightInDeg / 2;
			} else {
			...
			me._planta.ggMap.setView(newCenter, me._planta.ggMap.getZoom(), {animate: false});
			me._planta.ggInCheckBounds = false;
		}
		...
		player.addListener('configloaded', function() {
			me._planta.ggClearMap();
			me._planta.ggInitMap(false);
			me._planta.ggInitMapMarkers(true);
		});
	};
	...
	me.skinTimerEvent=function() {
		...
		me._planta.ggUpdateConditionTimer();
	};
	...
	function SkinHotspotClass_ht_node(parentScope,hotspot) {
		...
		hs='data:image/svg+xml;base64,'+
	...
	function SkinElement_marcador_negro_Class(parentScope,ggParent) {
		var me=this;
		var flag=false;
		me.parentScope=parentScope;
		...
		el=me._marcador_negro=document.createElement('div');
		els=me._marcador_negro__img=document.createElement('img');
		els.className='ggskin ggskin_marcador_negro';
		hs=basePath + 'images/marcador_negro.png';
		els.setAttribute('src',hs);
		els.ggNormalSrc=hs;
		els.setAttribute('style','position: absolute;top: 0px;left: 0px;width: 100%;height: 100%;-webkit-user-drag:none;pointer-events:none;;');
		...
		el.setAttribute('style',hs);
		el.style[domTransform + 'Origin']='50% 50%';
		me._marcador_negro.ggIsActive=function() {
			return player.getCurrentNode()==this.ggElementNodeId();
		}
		el.ggElementNodeId=function() {
			return me.ggNodeId;
		}
		me._marcador_negro.onclick=function (e) {
			player.openNext("{"+me.ggNodeId+"}","");
		}
		me._marcador_negro.ggUpdatePosition=function (useTransition) {
		}
	};
	me.addSkin();
	...
	me._plantas.logicBlock_visible();
